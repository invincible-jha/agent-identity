"""HTTP server for agent-identity using stdlib http.server.

Routes:
    POST   /identities         — register a new agent identity
    POST   /verify             — verify agent identity and capabilities
    GET    /trust/{id}         — get trust score for an agent
    GET    /health             — health check

Usage:
    python -m agent_identity.server.app --port 8080
    python -m agent_identity.server.app --host 127.0.0.1 --port 9000
"""
...
# URL pattern for /trust/{id}
_TRUST_ID_PATTERN = re.compile(r"^/trust/([^/]+)$")


class AgentIdentityHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the agent-identity server.
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            ...
            match = _TRUST_ID_PATTERN.match(path)
            if match:
                agent_id = match.group(1)
                status, data = routes.handle_get_trust(agent_id)
                self._send_json(status, data)
            else:
                self._send_json(
                    404,
...
def create_server(host: str = "0.0.0.0", port: int = 8080) -> HTTPServer:
    """Create (but do not start) the agent-identity HTTP server.
    ...
    logger.info("agent-identity server created at http://%s:%d", host, port)
    return server


def run_server(host: str = "0.0.0.0", port: int = 8080) -> None:
    """Create and run the agent-identity HTTP server (blocking).
    ...
    logger.info("Serving agent-identity on http://%s:%d — press Ctrl-C to stop", host, port)
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        logger.info("Shutting down agent-identity server.")
    finally:
        server.server_close()
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="agent-identity HTTP server",
