...
```bash
pip install "agent-identity[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentTrust control plane:

```python
# Before (open-source)
from agent_identity.core import Settings
...
The AgentTrust SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_identity.plugins.registry import processor_registry
processor_registry.load_entrypoints("agent_identity.plugins")
```
