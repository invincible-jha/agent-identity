...
This document describes the high-level architecture of agent-identity
...
agent-identity uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentTrust enterprise edition) to extend the system
...
from agent_identity.core import BaseProcessor  # example base class
